...
class SpacyPreprocessor(GraphComponent):
    ...
    def doc_for_text(self, model: Language, text: Text) -> Doc:
        """Makes a spaCy doc object from a string of text."""
        return model(self.preprocess_text(text))
    ...
    def process_content_bearing_samples(
        self, model: Language, samples_to_pipe: List[Tuple[int, Text]]
    ) -> List[Tuple[int, Doc]]:
    ...
    def process_non_content_bearing_samples(
        self, model: Language, empty_samples: List[Tuple[int, Text]]
    ) -> List[Tuple[int, Doc]]:
    ...
    def process_training_data(self, training_data: TrainingData, spacy_model: SpacyModel) -> None:
        ...
        attribute_docs = self.docs_for_training_data(model, training_data)
            ...
            for attribute in DENSE_FEATURIZABLE_ATTRIBUTES:
                if message.get(attribute):
                    message.set(
                        ...
                        self.doc_for_text(model, message.get(attribute)),
                    )
